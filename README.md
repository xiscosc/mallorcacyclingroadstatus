...
`adapter-cloudflare` v7 overwrites the file at its config's `main`, so the adapter is pointed at
a build-only config (`wrangler.build.jsonc`) instead of `wrangler.toml` to keep it from clobbering
the `src/worker.ts` wrapper (which is what adds `scheduled`). See `CLAUDE.md` for details — don't
delete `wrangler.build.jsonc`.
